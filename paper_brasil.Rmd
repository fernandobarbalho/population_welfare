...
const_u<- 4.800666 #Confomre cálculo feito em paper.R
const_c<- 38536.32 #Confomre cálculo feito em paper.R
...
dados_calculo_paper %>%
  filter(populacao < 5) %>%
  ggplot(aes(x=as.factor(ano), y=populacao)) +
  geom_line(aes(group = sigla_uf)) 
```

```{r}
dados_calculo_paper %>%
  ggplot(aes(x=sigla_uf, y=gC)) +
  geom_boxplot()
